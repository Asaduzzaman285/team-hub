...
Create a `.env` file in `team-hub/apps/api`:
...
```bash
# Push schema to DB
cd team-hub/apps/api && npx prisma db push

# Start both frontend and backend from the team-hub root
cd team-hub && npm run dev
```
